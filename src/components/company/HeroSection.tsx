...
export function HeroSection({ company, onActionClick }: HeroSectionProps) {
    return (
        <div className="bg-white dark:bg-slate-950 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden p-6 md:p-8 flex flex-col lg:flex-row gap-8 relative transition-colors duration-300">
            ...
            <div className="flex-1 space-y-6 z-10">
                <div className="flex gap-6 items-start">
                    ...
                    <div>
                        ...
                        <h1 className="text-3xl md:text-5xl font-extrabold text-slate-900 dark:text-white tracking-tight">{company.name}</h1>
                        <p className="text-slate-500 dark:text-slate-400 text-lg mt-1">{company.short_name} • {company.nature_of_company}</p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6 pt-2">
                    <div className="space-y-1">
                        ...
                        <p className="text-slate-900 dark:text-slate-200 font-semibold">{company.headquarters_address.split(',')[0]}</p>
                    </div>
                    <div className="space-y-1">
                        <div className="flex items-center gap-2 text-slate-400 dark:text-slate-500 text-sm font-medium">
                            <Users className="w-4 h-4" /> Global Size
                        </div>
                        <p className="text-slate-900 dark:text-slate-200 font-semibold">{company.employee_size}</p>
                    </div>
                    <div className="space-y-1">
                        <div className="flex items-center gap-2 text-slate-400 dark:text-slate-500 text-sm font-medium">
                            <MapPin className="w-4 h-4" /> Locations
                        </div>
                        <p className="text-slate-900 dark:text-slate-200 font-semibold">{company.office_locations.split(';').length} Countries</p>
                    </div>
                    <div className="space-y-1">
                        <div className="flex items-center gap-2 text-slate-400 dark:text-slate-500 text-sm font-medium">
                        ...
                        <a href={company.website_url} target="_blank" rel="noopener noreferrer" className="text-primary font-semibold hover:underline truncate block">
                            {new URL(company.website_url || 'https://google.com').hostname}
                        </a>
                    </div>
                </div>
            ...
            {/* Right: Quick Action Cards (Skills, InnovX, Hiring) */}
            <div className="flex flex-col gap-3 min-w-[240px] z-10">
                <ActionCard
                    title="Skills Matrix"
                    subtitle="Bloom's Taxonomy Map"
                    color="bg-indigo-50 hover:bg-indigo-100 dark:bg-indigo-950/30 dark:hover:bg-indigo-900/40"
                    textColor="text-indigo-700 dark:text-indigo-300"
                    borderColor="dark:border-indigo-900/50"
                    icon="📊"
                ...
                <ActionCard
                    ...
                    color="bg-amber-50 hover:bg-amber-100 dark:bg-amber-950/30 dark:hover:bg-amber-900/40"
                    textColor="text-amber-700 dark:text-amber-300"
                    borderColor="dark:border-amber-900/50"
                    icon="💡"
                    onClick={() => onActionClick("innovx")}
                />
                <ActionCard
                    title="Hiring Rounds"
                    subtitle="Process & Prep"
                    color="bg-emerald-50 hover:bg-emerald-100 dark:bg-emerald-950/30 dark:hover:bg-emerald-900/40"
                    textColor="text-emerald-700 dark:text-emerald-300"
                    borderColor="dark:border-emerald-900/50"
                    icon="🎯"
                    onClick={() => onActionClick("hiring")}
                />
            </div>
        </div>
    );
